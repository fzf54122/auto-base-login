...
logger = get_logger(LoginStrategyType.TOUTIAO)


class ToutiaoLogin(LoginStrategy):
    name = LoginStrategyType.TOUTIAO
    ...
    async def handler_auth_cookie(self, page: Page, account_file: str, options: LoginOptions) -> LoginResponse:
        ...
        return await self._optimized_auth_check(page, account_file, options,
                                                url=settings.TOUTIAO_HOME_URL,
                                                need_login_text=settings.TOUTIAO_NEED_LOGIN_TEXT)
    ...
    async def handler_login_and_save(self, page: Page, account_file: str, options: LoginOptions) -> LoginResponse:
        ...
        # 打开登录页
        await self.safe_goto(page, settings.TOUTIAO_LOGIN_URL, options)
        try:
            # ✅ 等到登录成功自动跳到 home（最多等 timeout_ms）
            await page.wait_for_url(settings.TOUTIAO_HOME_URL, timeout=options.timeout_ms)
            # 保存 cookie
